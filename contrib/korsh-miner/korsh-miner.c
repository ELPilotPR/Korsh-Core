 ...
 *
 * Build: see build.sh.   Usage: korsh-miner <payout address> [--threads N] [--conf FILE] [--rpc URL]
 */
#define _GNU_SOURCE
#include <curl/curl.h>
#include <errno.h>
...
/* Optional two-way interleaved kernel (korsh-yp2.c). Built with -DKORSH_NO_YP2 it is replaced by a stub. */
#ifdef KORSH_NO_YP2
static int korsh_yp2_supported(void) { return 0; }
static int korsh_yespower_hash2(const uint8_t *a, const uint8_t *b, uint8_t *oa, uint8_t *ob)
{
...
static void hash_nonces(uint8_t h[80], uint32_t n, uint32_t n2, uint8_t out[2][32])
{
    if (g_ways == 2) {
        ...
        put_le32(h + 76, n);
        put_le32(h2 + 76, n2);
        if (korsh_yespower_hash2(h, h2, out[0], out[1]) == 0) return;
...
{
    if (!korsh_yp2_supported()) return 0;
    ...
    for (int p = 0; p < pairs; p++) {
        ...
        put_le32(a + 68, 1789866060u + (uint32_t)p);   /* after the yespower 1.0 switch */
        put_le32(b + 68, 1789866060u + (uint32_t)p * 7u);
        if (korsh_yespower_hash2(a, b, oa, ob) != 0) return 0;
...
static int build_block(json_t *t, const uint8_t spk[25], uint64_t extranonce, block_t *b, char *err, size_t errlen)
{
    ...
    for (int k = 0; k < 2; k++) {
        json_t *v = json_object_get(t, keys[k]);
        if (v && ((json_is_array(v) && json_array_size(v)) || (json_is_object(v) && json_object_size(v)))) {
            snprintf(err, errlen, "template requires %s payments (unsupported)", keys[k]);
            return -1;
        }
    }
    int height = (int)json_integer_value(json_object_get(t, "height"));
    int64_t value = (int64_t)json_integer_value(json_object_get(t, "coinbasevalue"));
...
static int run_stratum(const char *url, const char *user, const char *pass)
{
    ...
    while (!g_stop) {
        ...
        sjob_t cur = {0};
        int ok = send_json(c, json_pack("{s:i,s:s,s:[s]}", "id", 1, "method", "mining.subscribe", "params", "korsh-miner/1.0")) == 0;
...
/* header with a timestamp after the yespower 1.0 activation, so the consensus (1.0) parameters are used */
static void bench_header(uint8_t h[80])
{
    memset(h, 0, 80);
    memcpy(h + 4, "korsh-benchmark-header", 22);
    put_le32(h + 68, 1789866060u);
}
...
int main(int argc, char **argv)
{
    ...
    if (ways_opt == 1) {
        g_ways = 1;
    } else if (korsh_yp2_supported() && yp2_check(16)) {
        g_ways = 2;
    } else {
        g_ways = 1;
    ...
    if (surl) { /* pool mining: no local node needed */
        ...
        printf("korsh-miner: %d threads, %d-way kernel -> %s (stratum, %s difficulty-1)\n", g_threads, g_ways, surl, g_diff1_bitcoin ? "bitcoin" : "scrypt");
        fflush(stdout);
    ...
    snprintf(g_url, sizeof g_url, "%s", rpc_url ? rpc_url : "http://127.0.0.1:8382/");
    ...
    printf("korsh-miner: %d threads, %d-way kernel -> %s\n", g_threads, g_ways, addr);
    fflush(stdout);
