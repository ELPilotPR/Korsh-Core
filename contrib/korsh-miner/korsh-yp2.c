/*
 * korsh-yp2.c: two-way interleaved Yespower 1.0 kernel for the Korsh miner.
 *
 ...
 * bit-identical results to the reference (korsh-miner --selftest) and falls back to the reference otherwise.
 *
 * Scope: yespower 1.0, no personalization (what Korsh uses), N and r fixed to the values below.
 */
...
#define KYP_N 256u
#define KYP_R 8u
#define KYP_PERS_TIME_THRESHOLD 1546539305u   /* same switch to 1.0 as the node's yespower.c */
...
int korsh_yespower_hash2(const uint8_t *in0, const uint8_t *in1, uint8_t *out0, uint8_t *out1)
{
	...
	/* only yespower 1.0 is implemented (the node switches to it after this timestamp) */
