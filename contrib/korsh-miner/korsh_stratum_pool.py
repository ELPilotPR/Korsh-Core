#!/usr/bin/env python3
"""Minimal Stratum v1 reference pool for a Korsh node (testing / development tool).
...
It exists to exercise Stratum clients end to end and to document the protocol conventions:
  ...
  * share validation .. the node's own yespower_hash() (via libyp.so), not a re-implementation
  ...
  * ntime / nonce ..... sent and received as %08x of the numeric header field
    ...
    ./korsh_stratum_pool.py <pool payout address> [--listen 127.0.0.1:3333] [--ease 16]
...
class Pool:
    def __init__(self, node, address, ease):
        ...
        lib = ctypes.CDLL(os.path.join(HERE, "libyp.so"))
