#!/usr/bin/env python3
"""Korsh solo CPU miner (Yespower) for the Korsh node's getblocktemplate/submitblock RPC.

The hash function is the node's own yespower_hash() (built by build.sh into libyp.so), so a
nonce found here is by construction valid for the node's consensus code.

    ./korsh_miner.py <payout address> [--threads N] [--conf PATH] [--rpc URL]
"""
import argparse
import base64
import ctypes
...
def build_block_parts(t, spk, extranonce):
    ...
    sig = height_script(t["height"]) + push(struct.pack("<Q", extranonce)) + push(b"korsh")
...
def main():
    ...
    hashes = blocks = 0
    t0 = time.time()
    print("korsh-miner: %d threads -> %s" % (args.threads, args.address), flush=True)
